# Tiling 23 — Mouse Interactions

> Status: ✅ Normative

## Purpose

Mouse monitor/drag state turn completed user resize or move gestures into actor messages; a move completion requests return to tiled layout.

## Scope

- Owns the actor boundary described below: `UserResizeCompleted` and `UserMoveCompleted`..
- Owns the corresponding read boundary: No dedicated query..
...
| ------------------- | ------------------------------------------- | ----------------------------------------------------- | ---------------------------------- | ----------------------- | -------------------- | --------------------------- |
| UserResizeCompleted | `events/mouse_monitor.rs` / `drag_state.rs` | `handle_message` → `on_user_resize_completed`         | updates ratios from old/new frames | layout subscriber       | no public route      | None — source-only evidence |
| UserMoveCompleted   | `events/mouse_monitor.rs` / `drag_state.rs` | `handle_message` → subscriber `notify_layout_changed` | no actor state mutation            | snap-back layout effect | no public route      | None — source-only evidence |
...
- Producer: `events/{mouse_monitor,drag_state}.rs` and `actor/handlers/resize.rs`.
- Actor input: `UserResizeCompleted` and `UserMoveCompleted`..
...
- Public transport, if any: No dedicated Tauri command/event..
...
No dedicated Tauri command/event.
...
Resize completion can update ratios; move completion triggers layout refresh.
...
1. **Normal.** Given a running actor and a valid `UserResizeCompleted` and `UserMoveCompleted`. producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
The stable seam is the actor message/query boundary in `app/native/src/modules/tiling/actor/messages.rs`, with focused handler/module tests adjacent to `events/{mouse_monitor,drag_state}.rs` and `actor/handlers/resize.rs`. Existing evidence is **None — source-only evidence** unless a focused test is named in the Evidence Base; a rewrite should test observable handler/effect behavior rather than source text.
...
| Contract surface | Implementation evidence                                               | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | --------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `events/{mouse_monitor,drag_state}.rs` and `actor/handlers/resize.rs` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | No dedicated Tauri command/event.                                     | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
