# Tiling 07 — Initial Reconciliation

> Status: ✅ Normative

## Purpose

Startup enumerates candidate windows, sends a creation batch, and then sends `InitComplete`; this is sequencing, not a registry-wide completion barrier.

## Scope

- Owns the actor boundary described below: `StateMessage::BatchWindowsCreated` and `StateMessage::InitComplete`.
- Owns the corresponding read boundary: `StateQuery::GetVisibleWorkspaces`.
...
| ------------------- | --------------------- | --------------------------------------------- | ---------------------------------------------------------------- | --------------------------------------- | ----------------------------------------------------- | --------------------------- |
| BatchWindowsCreated | `init.rs` enumeration | `handle_message` → `on_batch_windows_created` | tracks startup candidates without individual layout notification | no frame effect until completion        | startup-only                                          | None — source-only evidence |
| InitComplete        | `init.rs`             | `handle_message` → `on_init_complete`         | marks bootstrap complete                                         | notifies layouts for visible workspaces | `stache://tiling/initialized` is emitted by `init.rs` | None — source-only evidence |
...
- Producer: `init.rs::{initialize_tiling,initialize_state}` and `actor/handlers/window.rs`.
- Actor input: `StateMessage::BatchWindowsCreated` and `StateMessage::InitComplete`.
...
- Public transport, if any: `stache://tiling/initialized` is the public lifecycle notification..
...
`stache://tiling/initialized` is the public lifecycle notification.
...
Visible-workspace layouts run after initialization completion.
...
1. **Normal.** Given a running actor and a valid `StateMessage::BatchWindowsCreated` and `StateMessage::InitComplete` producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
| Contract surface | Implementation evidence                                                        | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | ------------------------------------------------------------------------------ | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `init.rs::{initialize_tiling,initialize_state}` and `actor/handlers/window.rs` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | `stache://tiling/initialized` is the public lifecycle notification.            | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
