//! Thread utilities for spawning named threads and GCD dispatch.
//!
//! This module provides utilities for:
//! - Spawning named threads with the `stache-` prefix
//! - Dispatching work to the main thread via GCD
//! - Dispatching work to high-priority background queues

use std::ffi::c_void;
use std::thread;

/// Spawns a named thread with the `stache-` prefix.
///
/// The thread name will be `stache-{name}`, which helps identify threads
/// in debugging and profiling tools.
///
/// # Example
///
/// ```ignore
/// spawn_named_thread("audio-watcher", || {
///     // This runs in a thread named "stache-audio-watcher"
/// });
/// ```
...
where F: FnOnce() + Send + 'static {
    ...
    if let Err(err) = thread::Builder::new().name(thread_name.clone()).spawn(task) {
        tracing::error!("Failed to spawn {thread_name}: {err}");
    }
}
