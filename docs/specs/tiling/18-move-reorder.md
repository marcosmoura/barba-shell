# Tiling 18 — Move and Reorder

> Status: ✅ Normative

## Purpose

Reordering is actor state mutation through exact `WindowTarget` swaps or focused directional swap commands.

## Scope

- Owns the actor boundary described below: `SwapWindows`, `SwapWindowInDirection`.
- Owns the corresponding read boundary: Workspace window-order queries..
...
| ----------- | ----------------------------- | ------------------------------------ | -------------------------- | ---------------------- | -------------------- | --------------------------- |
| SwapWindows | internal exact-target command | `handle_message` → `on_swap_windows` | reorders two exact targets | layout subscriber      | CLI window `--swap`  | None — source-only evidence |
...
- Producer: `actor/handlers/window_move.rs` and `actor/handlers/focus.rs`.
- Actor input: `SwapWindows`, `SwapWindowInDirection`.
...
- Public transport, if any: CLI/hotkey transport maps to actor handles; no dedicated emitted reorder event..
- CLI `tiling window` applies `--swap` after `--focus` and before preset/resize/send (`app/native/src/cli/commands/tiling.rs::execute_window`).
...
CLI/hotkey transport maps to actor handles; no dedicated emitted reorder event.
...
A successful mutation requests layout recomputation.
...
1. **Normal.** Given a running actor and a valid `SwapWindows`, `SwapWindowInDirection` producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
| Contract surface | Implementation evidence                                                         | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | ------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `actor/handlers/window_move.rs` and `actor/handlers/focus.rs`                   | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | CLI/hotkey transport maps to actor handles; no dedicated emitted reorder event. | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
