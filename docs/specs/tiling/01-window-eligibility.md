# Tiling 01 — Window Eligibility

> Status: ✅ Normative

## Purpose

`WindowCreated` is admitted only through the live eligibility/rule path; rejected windows are not tracked as layout members.

## Scope

- Owns the actor boundary described below: `StateMessage::WindowCreated`.
- Owns the corresponding read boundary: `StateQuery::GetLayoutableWindows`, `GetLayoutableWindowIds`.
...
| ---------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Adjacent tiling state policy | [tiling/02-window-enumeration.md](02-window-enumeration.md)                     | This capability consumes that state rather than redefining it. |
...
| ------------- | ------------------------------------------ | ------------------------------------------------ | ------------------------------------ | ----------------------------------------- | ------------------------------------------------------- | --------------------------- |
| WindowCreated | `events/processor.rs` / initial enumerator | `handle_message` → `handlers::on_window_created` | adds eligible window/rule assignment | subscriber may recompute workspace layout | `stache://tiling/window-tracked`; no standalone command | None — source-only evidence |
...
- Producer: `app/native/src/modules/tiling/rules/mod.rs` and `actor/handlers/window.rs::on_window_created`.
- Actor input: `StateMessage::WindowCreated`.
...
- Public transport, if any: No standalone public command; CLI/Tauri commands consume the tracked set..
...
No standalone public command; CLI/Tauri commands consume the tracked set.
...
None directly; admission can request a later workspace layout.
...
1. **Normal.** Given a running actor and a valid `StateMessage::WindowCreated` producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
The stable seam is the actor message/query boundary in `app/native/src/modules/tiling/actor/messages.rs`, with focused handler/module tests adjacent to `app/native/src/modules/tiling/rules/mod.rs` and `actor/handlers/window.rs::on_window_created`. Existing evidence is **None — source-only evidence** unless a focused test is named in the Evidence Base; a rewrite should test observable handler/effect behavior rather than source text.
...
| Contract surface | Implementation evidence                                                                        | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | ---------------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `app/native/src/modules/tiling/rules/mod.rs` and `actor/handlers/window.rs::on_window_created` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | No standalone public command; CLI/Tauri commands consume the tracked set.                      | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
