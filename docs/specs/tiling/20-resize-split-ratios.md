# Tiling 20 — Resize and Split Ratios

> Status: ✅ Normative

## Purpose

Resize inputs mutate workspace split ratios by workspace UUID/index or focused-window dimension, then cause layout recomputation.

## Scope

- Owns the actor boundary described below: `ResizeSplit` and `ResizeFocusedWindow`..
...
| ---------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Adjacent tiling state policy | [tiling/15-minimum-size-constraints.md](15-minimum-size-constraints.md)         | This capability consumes that state rather than redefining it. |
...
| ------------------- | -------------------- | --------------------------------------------- | -------------------------------- | ---------------------- | -------------------- | ------------------------------------------------------- |
| ResizeSplit         | IPC/CLI actor handle | `handle_message` → `on_resize_split`          | updates workspace ratio at index | layout subscriber      | CLI window resize    | None — source-only evidence                             |
| ResizeFocusedWindow | IPC/CLI actor handle | `handle_message` → `on_resize_focused_window` | updates focused layout ratio     | layout subscriber      | CLI window resize    | `actor/messages.rs::tests::test_resize_dimension_parse` |
...
- Producer: `actor/handlers/resize.rs`, `actor/messages.rs::ResizeDimension`, and `layout/{split,dwindle,master}.rs`.
- Actor input: `ResizeSplit` and `ResizeFocusedWindow`..
...
- Public transport, if any: CLI/hotkey adapters parse resize dimensions; no ratio event is declared..
- CLI `tiling window` applies resize before send, following `app/native/src/cli/commands/tiling.rs::execute_window`.
...
CLI/hotkey adapters parse resize dimensions; no ratio event is declared.
...
New ratios produce later frame effects.
...
1. **Normal.** Given a running actor and a valid `ResizeSplit` and `ResizeFocusedWindow`. producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
The stable seam is the actor message/query boundary in `app/native/src/modules/tiling/actor/messages.rs`, with focused handler/module tests adjacent to `actor/handlers/resize.rs`, `actor/messages.rs::ResizeDimension`, and `layout/{split,dwindle,master}.rs`. Existing evidence is **None — source-only evidence** unless a focused test is named in the Evidence Base; a rewrite should test observable handler/effect behavior rather than source text.
...
| Contract surface | Implementation evidence                                                                                  | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | -------------------------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `actor/handlers/resize.rs`, `actor/messages.rs::ResizeDimension`, and `layout/{split,dwindle,master}.rs` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | CLI/hotkey adapters parse resize dimensions; no ratio event is declared.                                 | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
