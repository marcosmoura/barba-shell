...
/// Event name emitted when user clicks outside the widgets window.
pub const CLICK_OUTSIDE_EVENT: &str = "widgets:click-outside";
...
extern "C" fn mouse_event_callback(
    ...
    if !frame.contains(location) {
        let _ = app_handle.emit(CLICK_OUTSIDE_EVENT, ());
    }
