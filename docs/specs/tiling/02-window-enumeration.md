# Tiling 02 — Window Enumeration

> Status: ✅ Normative

## Purpose

Initial enumeration supplies currently discoverable native windows to the bootstrap path; it is not a typed partial-snapshot protocol.

## Scope

- Owns the actor boundary described below: `StateMessage::BatchWindowsCreated`.
- Owns the corresponding read boundary: `StateQuery::GetAllWindows`.
...
- Producer: `app/native/src/modules/tiling/init.rs` and `ffi/window_query.rs`.
- Actor input: `StateMessage::BatchWindowsCreated`.
...
- Public transport, if any: No direct UI invoke; startup is the boundary..
...
No direct UI invoke; startup is the boundary.
...
Creates candidate state only; layout is deferred to initialization completion.
...
1. **Normal.** Given a running actor and a valid `StateMessage::BatchWindowsCreated` producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
The stable seam is the actor message/query boundary in `app/native/src/modules/tiling/actor/messages.rs`, with focused handler/module tests adjacent to `app/native/src/modules/tiling/init.rs` and `ffi/window_query.rs`. Existing evidence is **None — source-only evidence** unless a focused test is named in the Evidence Base; a rewrite should test observable handler/effect behavior rather than source text.
...
| Contract surface | Implementation evidence                                           | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | ----------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `app/native/src/modules/tiling/init.rs` and `ffi/window_query.rs` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | No direct UI invoke; startup is the boundary.                     | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
