# Tiling 25 — Hidden App Restoration

> Status: ✅ Normative

## Purpose

Application hide/show events are revalidated against native hidden state before actor visibility policy is updated.

## Scope

- Owns the actor boundary described below: `AppHidden` and `AppShown`..
...
| ---------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Adjacent tiling state policy | [tiling/24-visibility-app-hiding.md](24-visibility-app-hiding.md)               | This capability consumes that state rather than redefining it. |
...
| ------------- | ----------------------- | -------------------------------------------- | ---------------------------------------- | ------------------------------------- | -------------------- | --------------------------- |
| AppTerminated | `events/app_monitor.rs` | `handle_message` → `on_app_terminated_exact` | reconciles exact terminated app instance | visibility cleanup/restoration policy | no public route      | None — source-only evidence |
...
- Producer: `actor/mod.rs::{on_app_hidden_revalidated,on_app_shown_revalidated}` and `visibility.rs`.
- Actor input: `AppHidden` and `AppShown`..
...
- Public transport, if any: No dedicated public restoration event..
...
No dedicated public restoration event.
...
Visibility policy may request native restoration.
...
1. **Normal.** Given a running actor and a valid `AppHidden` and `AppShown`. producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
| Contract surface | Implementation evidence                                                                  | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | ---------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `actor/mod.rs::{on_app_hidden_revalidated,on_app_shown_revalidated}` and `visibility.rs` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | No dedicated public restoration event.                                                   | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
