# Tiling 06 — Event Normalization and Batching

> Status: ✅ Normative

## Purpose

The processor translates and batches geometry updates; a batch is delivered as observed rather than as a documented deterministic ordering fence.

## Scope

- Owns the actor boundary described below: `StateMessage::BatchedGeometryUpdates`.
...
| ---------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Adjacent tiling state policy | [tiling/05-event-observation.md](05-event-observation.md)                       | This capability consumes that state rather than redefining it. |
...
| ---------------------- | --------------------- | -------------------------------------------------------------------------------- | -------------------------------- | ------------------------------------------------------------------ | ---------------------- | --------------------------- |
| BatchedGeometryUpdates | `events/processor.rs` | `handle_message` filters each identity → `handlers::on_batched_geometry_updates` | updates accepted window geometry | later layout subscriber notification only when handler requests it | no direct public route | None — source-only evidence |
...
- Producer: `events/processor.rs::{EventProcessor,process_events}` and `actor/handlers/window.rs`.
- Actor input: `StateMessage::BatchedGeometryUpdates`.
...
- Public transport, if any: No direct Tauri/CLI surface..
...
Handlers update geometry and may request a later layout refresh.
...
1. **Normal.** Given a running actor and a valid `StateMessage::BatchedGeometryUpdates` producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
| Contract surface | Implementation evidence                                                               | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | ------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `events/processor.rs::{EventProcessor,process_events}` and `actor/handlers/window.rs` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | No direct Tauri/CLI surface.                                                          | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
