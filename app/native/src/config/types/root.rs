...
pub struct StacheConfig {
    ...
    /// `CapsLock+<key>` is handled as a Stache-only pseudo modifier: while any
    /// such binding is configured, Stache remaps the physical Caps Lock key to a
    /// spare function key at the HID layer, so tapping Caps Lock alone does
    /// nothing (capitalization never toggles and the Caps Lock LED stays off),
    /// while holding Caps Lock with a configured key executes the command.
