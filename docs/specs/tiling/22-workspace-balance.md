# Tiling 22 — Workspace Balance

> Status: ✅ Normative

## Purpose

Balancing requests equalized ratio state for a workspace before the selected layout computes frames.

## Scope

- Owns the actor boundary described below: `BalanceWorkspace`.
...
| ---------------- | -------------------------- | ----------------------------------------- | --------------------------- | ---------------------- | -------------------------------- | --------------------------- |
| BalanceWorkspace | CLI/workspace actor handle | `handle_message` → `on_balance_workspace` | normalizes workspace ratios | layout subscriber      | CLI `tiling workspace --balance` | None — source-only evidence |
...
- Producer: `actor/handlers/resize.rs::on_balance_workspace` and `layout/{split,dwindle,master}.rs`.
- Actor input: `BalanceWorkspace`.
...
- Public transport, if any: No dedicated Tauri event/result payload..
- CLI `tiling workspace` applies balance after layout and before send (`app/native/src/cli/commands/tiling.rs::execute_workspace`).
...
No dedicated Tauri event/result payload.
...
Balance triggers layout/frame recomputation.
...
1. **Normal.** Given a running actor and a valid `BalanceWorkspace` producer input, when the handler accepts it, then only the named state/effect boundary is used.
...
| Contract surface | Implementation evidence                                                                 | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | --------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `actor/handlers/resize.rs::on_balance_workspace` and `layout/{split,dwindle,master}.rs` | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | No dedicated Tauri event/result payload.                                                | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
