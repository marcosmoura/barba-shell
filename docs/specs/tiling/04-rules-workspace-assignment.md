# Tiling 04 — Rules and Workspace Assignment

> Status: ✅ Normative

## Purpose

Window rules classify a candidate and select assignment during creation; workspace state remains owned by the workspace spec.

## Scope

- Owns the actor boundary described below: `StateMessage::WindowCreated`.
- Owns the corresponding read boundary: `StateQuery::GetWorkspaceByName`, `GetWindowsForWorkspace`.
...
- Producer: `rules/mod.rs`, `config/types/workspaces.rs`, and `actor/handlers/window.rs`.
...
- Public transport, if any: Configuration is JSONC `workspaces`/window-rule input; command transport belongs to foundation CLI..
...
Configuration is JSONC `workspaces`/window-rule input; command transport belongs to foundation CLI.
...
A successful assignment can schedule workspace layout; rules do not apply native frames.
...
| Contract surface | Implementation evidence                                                                             | Test evidence               | Intended documentation                                            | Disposition  |
| ---------------- | --------------------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------- | ------------ |
| Actor contract   | `rules/mod.rs`, `config/types/workspaces.rs`, and `actor/handlers/window.rs`                        | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/tiling.md#Architecture` | Current-only |
| Public boundary  | Configuration is JSONC `workspaces`/window-rule input; command transport belongs to foundation CLI. | None — source-only evidence | `/Users/marcosmoura/Documents/stache-docs/events-and-ipc.md`      | Current-only |
